...
select last_name,first_name,sex from t_reserve_order_passenger where last_name='test';
# 如果SELECT和INSERT的列可以不同,使用的是列在表中的位置,如INSTER第2列对应SELECT的第5列
insert into t_reserve_order_passenger (last_name,national,card_num) select last_name,first_name,national from t_reserve_order_passenger where last_name='test';
```
