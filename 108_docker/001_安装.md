...
```bash
$ docker run -itd --name centos7 4d8a72201fa4 /bin/bash
```
...
$ docker pull arm64v8/mysql:8.3
$ docker run --name mysql8 -e MYSQL_ROOT_PASSWORD=123456 -p 3306:3306 -d arm64v8/mysql:8.3
```
...
docker run --name redis1 -p 6379:6379 -d redis:latest --requirepass 123456789
docker run --name redis2 -p 6380:6380 -d redis:latest --requirepass 123456789
??? M1处理器设置 ip 后不能访问
```
